...
class Baseline_Regression(nn.Module):
    def __init__(self, last_stride, model_path, neck, neck_feat, model_name, pretrain_choice,in_planes,global_feat_fc,train_last_base_params):
        super(Baseline_Regression, self).__init__()
        ...
        if self.neck == 'no':
            self.classifier = nn.Linear(self.in_planes, 3)
            # self.classifier = nn.Linear(self.in_planes, self.num_classes, bias=False)     # new add by luo
            # self.classifier.apply(weights_init_classifier)  # new add by luo
        elif self.neck == 'bnneck':
            self.bottleneck = nn.BatchNorm1d(self.in_planes)
            self.bottleneck.bias.requires_grad_(False)  # no shift
            self.classifier = nn.Linear(self.in_planes, 3, bias=False)
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            cls_score[:,0:2]=torch.tanh(cls_score[:,0:2])
            cls_score[:,2]=torch.sigmoid(cls_score[:,2])
